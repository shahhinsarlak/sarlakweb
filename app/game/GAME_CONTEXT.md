...
- **Sprites:** animated 64×64 pixel-art per machine (incl. an animated Capacitor
  battery), muted body + vivid accent, in compact RLE/palette JSON
  ...
  via the PXLS `compositeToCanvas`. (Phase B will move these to 128×128 office-junk
  art with per-upgrade attachments.)
...
| Factory sprites | 64x64, 4 frames, RLE JSON, drawn via PXLS renderer (128x128 office-junk in Phase B) |
